...
def plotAb2D(A, b, fig=None, fs=14):
    if fig==None:
        fig = plt.figure(figsize=(6,6), num='Figure 1') 
    ax = fig.gca()
    xl = -4
    xr = 6
...
#     ax.spines['bottom'].set_smart_bounds(True)
    ...
    plt.text(0, 4.2, r'$x_2$', fontsize = 14)
    plt.text(-4.5, 0, r'$x_1$', fontsize = 14)
...
def fig_A():
    ...
    fig = plt.figure(figsize=(10,4), num='Figure A')
    ...
    eq1 = ax1.text(-2, 4.6, r'$%.1fx_1 + %.1fx_2 = %.1f$'%(A_00, A_01, b_0), fontsize = 10, color = 'g')
    eq2 = ax1.text(-2, 4.1, r'$%.1fx_1 + %.1fx_2 = %.1f$'%(A_10, A_11, b_1), fontsize = 10, color = 'b')
    ...
    ax2.set_zlabel(r'$f(x)$', fontsize = 14)
...
def sliders_figA(hdls):
    ...
    def update_plots(A_00, A_01, A_10, A_11, b_0, b_1, c_0):
        ...
        # update 3D surface
        [h.remove() for h in ax2.get_children() if isinstance(h, mpl_toolkits.mplot3d.art3d.Poly3DCollection)]
        ax2.plot_surface(x1, x2, zs, rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0, alpha=0.8)
    ...
    form = ipw.Box([ipw.Box([A_00, A_10, b_0, c_0], layout=v_layout), ipw.Box([A_01, A_11, b_1], layout=v_layout)], layout=h_layout)
    display(form)
...
def fig6(A, b, c):
    ...
    plt.text(2, -1.5, r'$x$', fontsize = 10)
    ...
    plt.text(0.1, 110, r'$f(x[i] + \alpha r[i])$', fontsize = 14)
    ax.set_title('(c)')
    ...
    plt.text(0.5, -1, r'$x[1]$', fontsize = 10)
...
def fig7(A, b, c):
    fig = plt.figure(figsize=(6,6), num='Figure 7')
    ...
    plt.plot(x[0,0], x[1,0], 'k', marker='o', markersize=6)
    plt.plot(2, -2, color='grey', marker='o', markersize=6)
    plt.text(2, -1.8, r'$x$', fontsize = 14)
    for i in range(2,7):
        xnew = x + 0.25*i*alpha*r
        rnew = b - A*xnew
        rnew /= rnew.T*rnew
        rp = float(r.T*rnew)
        ax.arrow(xnew[0,0], xnew[1,0], -rnew[0,0]*3, -rnew[1,0]*3, color = 'k', lw=1, width=.01)
        if abs(rp) > 0.01:
            ax.arrow(xnew[0,0], xnew[1,0], -r[0,0]*3*rp/delta, -r[1,0]*3*rp/delta, color = 'k', linestyle = ':', lw=2, width=.01)
    plt.axis([-2,3,-3,2])
...
def fig_B():
    ...
    fig = plt.figure(figsize=(6,6), num='Figure B')
...
def fig9():
    fig = plt.figure(figsize=(8,2), num='Figure 9')
    ...
    ax.text(0.65, 0.25, r'$v$', fontsize=12)
    plt.axis([-1, 1, -1, 1])
    v = B*v
    ...
    ax.yaxis.set_ticklabels('')
    plt.axis([-1, 1, -1, 1])
    ax.text(-0.45, -0.7, r'$Bv$', fontsize=12)
    v = B*v
    ...
    ax.yaxis.set_ticklabels('')
    plt.axis([-1, 1, -1, 1])
    ax.text(0.45, 0.45, r'$B^2v$', fontsize=12)
    v = B*v
    ...
    ax.yaxis.set_ticklabels('')
    plt.axis([-1, 1, -1, 1])
    ax.text(-0.55, -0.45, r'$B^3v$', fontsize=12)


def fig10():
    fig = plt.figure(figsize=(8,2), num='Figure 10')
    B = np.matrix([[1, -1], [-1, 1]])
    v = np.matrix([[-.11],[.11]])
    ax = fig.add_subplot(1,4,1)
    ax.arrow(0, 0, v[0,0], v[1,0], head_width = .12, head_length = .2, length_includes_head = True, color='k')
    ...
    ax.xaxis.set_ticklabels('')
    ax.yaxis.set_ticklabels('')
    ax.text(-0.45, 0.25, r'$v$', fontsize=12)
    plt.axis([-1, 1, -1, 1])
    v = B*v
    ax = fig.add_subplot(1,4,2)
    ax.arrow(0, 0, v[0,0], v[1,0], head_width = .12, head_length = .2, length_includes_head = True, color='k')
    ...
    ax.yaxis.set_ticklabels('')
    plt.axis([-1, 1, -1, 1])
    ax.text(-0.85, 0.25, r'$Bv$', fontsize=12)
    v = B*v
    ax = fig.add_subplot(1,4,3)
    ax.arrow(0, 0, v[0,0], v[1,0], head_width = .12, head_length = .2, length_includes_head = True, color='k')
    ax.spines['left'].set_position('zero')
    ...
    ax.yaxis.set_ticklabels('')
    plt.axis([-1, 1, -1, 1])
    ax.text(-0.95, 0.25, r'$B^2v$', fontsize=12)
    v = B*v
    ax = fig.add_subplot(1,4,4)
    ax.arrow(0, 0, v[0,0], v[1,0], head_width = .12, head_length = .2, length_includes_head = True, color='k')
    ax.spines['left'].set_position('zero')
    ...
    ax.yaxis.set_ticklabels('')
    plt.axis([-1, 1, -1, 1])
    ax.text(-0.95, 0.25, r'$B^3v$', fontsize=12)


def fig11():
    fig = plt.figure(figsize=(8,2), num='Figure 11')
    ...
    ax.xaxis.set_ticklabels('')
    ax.yaxis.set_ticklabels('')
    ax.text(-0.55, -1, r'$x$', fontsize=12)
    ax.text(-0.65, -0.3, r'$v_1$', fontsize=12)
    ax.text(0.1, -0.35, r'$v_2$', fontsize=12)
    plt.axis([-1, 1, -1, 1])
    x = B*x
    v1 = B*v1
    v2 = B*v2
    ...
    ax.yaxis.set_ticklabels('')
    plt.axis([-1, 1, -1, 1])
    ax.text(-0.95, -0.25, r'$Bx$', fontsize=12)
    x = B*x
    v1 = B*v1
    v2 = B*v2
    ax = fig.add_subplot(1,4,3)
    ...
    ax.yaxis.set_ticklabels('')
    plt.axis([-1, 1, -1, 1])
    ax.text(-0.55, -1.1, r'$B^2x$', fontsize=12)
    x = B*x
    v1 = B*v1
    v2 = B*v2
    ax = fig.add_subplot(1,4,4)
    ...
    ax.text(-1.25, 0.05, r'$B^3x$', fontsize=12)
...
def fig12(A, b, c):
    ...
    ax.arrow(2, -2, 1, 2, color = 'k', lw=1, width=.05)
    plt.axis([-4,6,-6,4])


def fig13():
    fig = plt.figure(figsize=(6,9), num='Figure 13')
    ...
    ax.text(-3.3, -2, r'$e_{[0]}$', fontsize=14)
    ax.spines['left'].set_position('zero')
    ax.spines['right'].set_color('none')
    ax.spines['bottom'].set_position('zero')
    ax.spines['top'].set_color('none')
    ...
    ax.text(2.2, -1, r'$e_{[1]}$', fontsize=14)
    ax.spines['left'].set_position('zero')
    ax.spines['right'].set_color('none')
    ax.spines['bottom'].set_position('zero')
    ax.spines['top'].set_color('none')
    ...
    ax.text(-0, -2.2, r'$e_{[2]}$', fontsize=14)
    ax.spines['left'].set_position('zero')
    ax.spines['right'].set_color('none')
    ax.spines['bottom'].set_position('zero')
    ax.spines['top'].set_color('none')
    ...
    ax.plot(steps[0,:], steps[1,:], '-o')
    ax.text(2.5, -2.2, r'$x$', fontsize=14)
    ax.text(-3.5, -2.5, r'$x_{[0]}$', fontsize=14)
...
def fig17():
    ...
    ax.set_zlabel(r'$\omega$', fontsize = 14)


def fig18():
    fig = plt.figure(figsize=(6,6), num='Figure 18')
    ax = fig.add_subplot(2,2,1)
...
def fig19():
    fig = plt.figure(figsize=(6,6), num='Figure 19')
    A = np.matrix([[3.0, 2.0], [2.0, 6.0]])
    b = np.matrix([[2.0], [-8.0]])
    c = 0.0
...
def fig20():
    plt.figure(figsize=(6,6), num='Figure 20')
    size = 99
    kappa = np.linspace(1, 100, size)
...
def fig21():
    fig = plt.figure(figsize=(6,6), num='Figure 21')
    A = np.matrix([[1.0, 0.0], [0.0, 1.0]])
    b = np.matrix([[2.0], [-2.0]])
    c = 0.0
    ...
    ax.arrow(2, -2, v2[0,0], v2[1,0], color = '#777777', head_width = .2, head_length = .3, length_includes_head = True)
    ax.text(-3.8, -3, r'$x_{[0]}$', fontsize=14)
    ax.text(2.1, -3, r'$x_{[1]}$', fontsize=14)
    ax.text(2.1, -2, r'$x$', fontsize=14)
    ax.text(-0.3, -3.4, r'$d_{[0]}$', fontsize=14)
    ax.text(1.4, -2.5, r'$e_{[1]}$', fontsize=14)


def fig22():
    fig = plt.figure(figsize=(9,4), num='Figure 22')
...
def fig23():
    fig = plt.figure(figsize=(9,4), num='Figure 23')    
    A = np.matrix([[3.0, 2.0], [2.0, 6.0]])
    b = np.matrix([[2], [-8]])
    c = 0.0
    x = np.matrix([[-2],[-2]])
    ...
    ax.text(-2.1, -2.4, r'$x_{[0]}$', fontsize=14)
    ax.text(2.1, -2, r'$x$', fontsize=14)
    ax.text(-2.9, -1.5, r'$d_{[0]}$', fontsize=14)
    ax.text(0, -1.2, r'$e_{[1]}$', fontsize=14)
    ax.set_title('(a)')
    ...
    ax.text(0, -2.5, r'$e_{[0]}$', fontsize=14)
    ax.set_title('(b)')


def fig24():
    fig = plt.figure(figsize=(8,2), num='Figure 24')
    u0 = np.matrix([[-1],[.5]])
    u1 = np.matrix([[0],[-1]])
    ...
    ax.text(-0.5, 0.35, r'$\mu_0$', fontsize=12)
    ax.text(0.05, -0.5, r'$\mu_1$', fontsize=12)
    ...
    ax.text(-0.5, 0.35, r'$d_{[0]}$', fontsize=12)
    ax.text(0.5, -0.1, r'$\mu^+$', fontsize=12)
    ax.text(0.5, -1, r'$\mu^*$', fontsize=12)
    ax.spines['left'].set_color('none')
    ax.spines['right'].set_color('none')
    ax.spines['bottom'].set_color('none')
    ax.spines['top'].set_color('none')
    ...
    ax.arrow(0, 0, u0[0,0], -u0[1,0], head_width = .12, head_length = .2, length_includes_head = True, color='k', ls=':')
    ax.text(-0.5, 0.35, r'$d_{[0]}$', fontsize=12)
    ax.text(-0.5, -0.5, r'$d_{[1]}$', fontsize=12)
    ax.spines['left'].set_color('none')
    ax.spines['right'].set_color('none')
    ax.spines['bottom'].set_color('none')
    ax.spines['top'].set_color('none')
...
def fig25():
    fig = plt.figure(figsize=(6,6), num='Figure 25')
    A = np.matrix([[3.0, 2.0], [2.0, 6.0]])
    b = np.matrix([[2.0], [-8.0]])
    c = 0.0
    d0 = np.matrix([[1],[0]])
...
def fig26():
    ...
    fig = plt.figure(figsize=(6,4.5), num='Figure 26')
    d0 = np.matrix([[2], [0]])
    d1 = np.matrix([[1], [1]])
    ax = fig.add_subplot(1, 1, 1)
...
def fig28():
    ...
    fig = plt.figure(figsize=(6,4.5), num='Figure 28')
    d0 = np.matrix([[2], [0]])
    d1 = np.matrix([[2], [1]])
    d2 = np.matrix([[-1], [1.25]])
    u0 = np.matrix([[2], [0]])
    ...
    ax.text(-2.2, -2.75, r'$d_{[0]}$', fontsize=12)
    ax.text(-3.25, -2., r'$d_{[1]}$', fontsize=12)
    ax.text(-0.8, -0.75, r'$d_{[2]}$', fontsize=12)
...
def fig29():
    from matplotlib.patches import Polygon
    fig = plt.figure(figsize=(6,4.5), num='Figure 29')
    d0 = np.matrix([[2], [0]])
    d1 = np.matrix([[2], [1]])
    ...
    ax.text(-2.2, -2.75, r'$d_{[0]}$', fontsize=12)
    ax.text(-3.25, -2., r'$d_{[1]}$', fontsize=12)
    ax.text(-1.65, -0.75, r'$d_{[2]}$', fontsize=12)
    ...
    ax.text(.9, -2.75, r'$r_{[0]}$', fontsize=12)
    ax.text(-1.25, -2.25, r'$r_{[1]}$', fontsize=12)
    ax.text(-2.9, -.75, r'$r_{[2]}$', fontsize=12)
    ax.arrow(2, -1.5, e2[0,0], e2[1,0], head_width = .12, head_length = .2, length_includes_head = True, color='k')
    ax.text(2.7, -.5, r'$e_{[2]}$', fontsize=12)
    ax.spines['left'].set_color('none')
    ax.spines['right'].set_color('none')
    ax.spines['bottom'].set_color('none')
    ax.spines['top'].set_color('none')
...
def fig_C():
    ...
    fig = plt.figure(figsize=(6,6), num='Figure C')
    ax1 = fig.add_subplot(1, 1, 1)
...
def fig31():
    fig = plt.figure(figsize=(6,6), num='Figure 31')
    ax = fig.add_subplot(2,2,1)
    ax.set_title('(a)')
    ax.plot([0, 10], [1, 1])
    ax.plot(2,1,'k.')
    ...
    ax.yaxis.set_ticks_position('left')
    ax.set_xticks([2,7])
    ax.text(10, -0.1, r'$\lambda$', fontsize=13)
    ax.text(-1, 1.1, r'$P_0(\lambda)$', fontsize=13)
    plt.axis([0,10,-1,1])
    ...
    ax.set_xticks([2,7])
    ax.text(10, -0.1, r'$\lambda$', fontsize=13)
    ax.text(-1, 1.1, r'$P_1(\lambda)$', fontsize=13)
    plt.axis([0,10,-1,1])
    ...
    ax.set_xticks([2,7])
    ax.text(10, -0.1, r'$\lambda$', fontsize=13)
    ax.text(-1, 1.1, r'$P_2(\lambda)$', fontsize=13)
    plt.axis([0,10,-1,1])
...
def fig32():
    ...
    fig = plt.figure(figsize=(6,6), num='Figure 32')
    ax = fig.add_subplot(2,2,1)
    ...
    ax.text(1.3, -0.1, r'$\omega$', fontsize=13)
...
def fig33():
    ...
    fig = plt.figure(figsize=(6,6), num='Figure 33')
    ax = fig.add_subplot(1,1,1)
    ...
    ax.text(8.3, -0.1, r'$\lambda$', fontsize=13)
    plt.axis([0,8,-1,1])


def fig34():
    plt.figure(figsize=(6,6), num='Figure 34')
    size = 99
    kappa = np.linspace(1, 100, size)
...
def fig35():
    plt.figure(figsize=(6,6), num='Figure 35')
    size = 99
